...
def run():
    ...
    ENTITY1 = data.Field(sequential=False, lower=True)
    ...
    dataset  = data.TabularDataset(path=config.TRAIN_DATASET_FNAME, 
                                    format='csv', 
                                    fields=[('entity1',ENTITY1),('text', TEXT),('label', LABEL)], 
                                    skip_header=True)
    # split the dataset, 8:2
#     train_data, valid_data = dataset.split(split_ratio=[0.8,0.2], random_state=random.getstate())
...
#                                     skip_header=True)
    ...
    ENTITY1.build_vocab(train_data, vectors=vectors_data)
    TEXT.build_vocab(train_data, vectors=vectors_data)
    LABEL.build_vocab(train_data,vectors=vectors_data)
...
#                                                     repeat=False)
    ...
    train_it = data.BucketIterator(train_data,
