...
#     dbpedia_test_df.to_json('../inputs/2021_dbpedia_0.1test.json', orient='records')
    ...
    dbpedia_train_df= dbpedia_train_df[dbpedia_train_df['bottom_level_type'].notna()]
    ...
    for i,row in dbpedia_train_resource_df.iterrows():
        # encode
        sent = row['question'] + str('[SEP]') + row['top_level_type']
        # sent = str('[CLS]') + row['question']

        if row['bottom_level_type'] not in bottom_label_to_id: continue
        labels.append(int(bottom_label_to_id[row['bottom_level_type']]))
    ...
    model = BertForSequenceClassification.from_pretrained(
                   'bert-base-uncased', 
                   num_labels = len(bottom_label_to_id), # The number of output labels. 2 for binary classification.  
    ...
    for epoch_i in range(0, epochs):
        ...
        for batch in validation_dataloader:
            ...
            # Unpack this training batch from our dataloader and copy each tensor to the GPU using 
            # `batch` contains three pytorch tensors:
            #   [0]: input ids 
            #   [1]: attention masks
            #   [2]: labels 
    ...
    df_stats.to_csv(output_dir + 'df_resources_bottom_stats.csv',index=None)
    models_dir = output_dir + 'BERT_resources_bottom/'
